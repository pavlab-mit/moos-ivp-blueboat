...
/*                                                          */
/*    2026-08: the loops now run on bb::Pid (see pid.h for  */
/*    the ScalarPID parity contract). The engine measures   */
/*    dt once per update() and feeds it to all three loops; */
/*    it owns no clock beyond the caller's curr_time.       */
/************************************************************/
...
class BBPIDEngine
{
 public:
  ...
  // --- Reference-FF lifecycle (the 27 Aug step-and-hold fixes) ---
  // The reference FF is the held derivative of the DESIRED heading; correct
  // for a helm's moving staircase, wrong once the command SETTLES (it forces
  // a standing error of ff/kp) and wrong for a repositioning step (a 180 deg
  // step's implied-rate sign is numeric noise). Both default OFF = legacy.
  //   ff_hold_time: zero the held derivative this long after the last
  //     command change (choose ~2x the helm cadence; 0 = hold forever).
  //   ff_step_limit_deg: command steps larger than this publish no FF at
  //     all -- that is a repositioning, not a trajectory (0 = no limit).
  void setFFHoldTime(double sec)      { m_ff_hold_time = sec; }
  void setFFStepLimitDeg(double deg)  { m_ff_step_limit_deg = deg; }
  double getFFHoldTime() const        { return m_ff_hold_time; }
  double getFFStepLimitDeg() const    { return m_ff_step_limit_deg; }

  // --- Integration lifecycle (authority-aware; brief section 3.2) ---
  // Cap on the dt credited to any single loop step (0 = off). A command
  // gap then earns ONE bounded increment instead of the whole gap's worth.
  void setMaxDt(double sec);
  double getMaxDt() const { return m_max_dt; }

  // false: freeze all three integrals (P/D stay live). Wire to
  // (BB_CMD_AUTHORITY == AUTONOMY && NVGR_STOP_REASON == NONE).
  void setIntegrateEnable(bool on);
  bool integrateEnabled() const { return m_integrate_enable; }

  // Tracking anti-windup: at each loop's own output clamp, and via
  // the engine's observation of its post-FF rails. Default OFF =
  // ScalarPID-verbatim at rails (integral parks at its own limit
  // behind the wall) so a parity replay is clean everywhere.
  void setAntiWindup(bool on);
  bool antiWindupEnabled() const { return m_antiwindup; }

  // Downstream rails, CONTRACT frame (surge > 0 forward, yaw > 0
  // starboard), -1/0/+1, sticky until changed: mixer saturation as
  // reported on the wire. The engine adds its own post-FF clamp rails
  // and maps yaw through rudder_polarity into the loop frame.
  void setExternalSaturation(int surge_sign, int yaw_sign)
       { m_ext_sat_surge = surge_sign; m_ext_sat_yaw = yaw_sign; }
  ...
  double speedKp()   const { return m_speed_pid.kp();   }
  double speedKi()   const { return m_speed_pid.ki();   }
  double speedKd()   const { return m_speed_pid.kd();   }
  double headingKp() const { return m_heading_pid.kp(); }
  double headingKi() const { return m_heading_pid.ki(); }
  double headingKd() const { return m_heading_pid.kd(); }
  double yawrateKp() const { return m_yawrate_pid.kp(); }
  double yawrateKi() const { return m_yawrate_pid.ki(); }
  double yawrateKd() const { return m_yawrate_pid.kd(); }

  // Integral terms (output units), for telemetry / tests.
  double speedITerm()   const { return m_speed_pid.i_term();   }
  double headingITerm() const { return m_heading_pid.i_term(); }
  double yawrateITerm() const { return m_yawrate_pid.i_term(); }
 ...
 protected:
  ...
  // PID cores (true positional PID; bb::Pid, natural Kp,Ki,Kd order)
  bb::Pid m_speed_pid;
  bb::Pid m_heading_pid;   // outer: heading error -> desired yaw rate
  bb::Pid m_yawrate_pid;   // inner: yaw-rate error -> rudder

  // One dt per update(), measured from consecutive curr_times and fed
  // to all three loops (each additionally clamps to max_dt).
  double m_prev_update_time;
  bool   m_have_update_time;
  double m_max_dt;
  bool   m_integrate_enable;
  bool   m_antiwindup;

  // Downstream rails: reported by the app (mixer saturation, contract
  // frame) and observed internally (last tick's post-FF output clamps).
  int m_ext_sat_surge, m_ext_sat_yaw;      // app-reported
  int m_rail_thrust,   m_rail_rudder;      // internal, last tick

  // Reference-FF lifecycle (see setter comments)
  double m_ff_hold_time;
  double m_ff_step_limit_deg;
